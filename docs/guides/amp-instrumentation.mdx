...
| CrewAI | 1.1.0 | LLM, Agent, Task | No tool spans are emitted; versions 1.14 and later are not installable alongside the pinned SDK |
| OpenAI (direct SDK) | 2.38.0 | LLM | — |
| Anthropic (direct SDK) | 0.45.0 | LLM | — |

:::warning CrewAI 1.14 and later will not install alongside the pinned SDK
CrewAI 1.14 caps `opentelemetry-api` at `>=1.34,<1.35`, while every AMP-instrumentation version above resolves `opentelemetry-api` to 1.43 or later. Those ranges cannot overlap, so `pip install` fails with `ResolutionImpossible` at install time rather than degrading at runtime. Stay on 1.1.x, the most recent release whose dependencies still resolve, until either project relaxes its pin.
:::
